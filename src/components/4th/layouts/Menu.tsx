...
const Menu = () => {
  ...
  return (
    <AnimatePresence>
      {isOpen ? (
        <div onClick={close} className="w-screen h-screen fixed top-0 left-0 bg-black bg-opacity-25">
          <motion.div
            initial={{
              scale: 0,
          ...
          >
            <nav className=" text-primary text-2xl flex justify-center" onClick={(e) => e.stopPropagation()}>
              <ul className="space-y-1 w-full ml-10">
                {menus.map((menu, index) => (
                  <li key={index} className='hover:text-white hover:before:content-[">"] focus:text-white'>
